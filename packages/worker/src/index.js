...
app.get('/', async (c) => {
  try {
    ...
    return c.json({ name: 'All Things AI API', version: '0.12.1', status: 'ok', db: 'connected' });
  } catch {
    return c.json({ name: 'All Things AI API', version: '0.12.1', status: 'degraded', db: 'error' }, 503);
  }
});
